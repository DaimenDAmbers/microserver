...
module ("templates.ScannerTemplate", thingworx.template.extend)
...
log.trace("[ScannerTemplate]","########### in GetSystemProperties#############")
    queryHardware()
...
--   set property temperature
     ...
     log.debug("[ScannerTemplate]",string.format("temp %.1f",s))
     properties.cpu_temperature.value = s
--  set property frequency
     s = freqCmd:read("*a")
     log.debug("[ScannerTemplate]",string.format("raw freq %s",s))
     s = string.match(s,"frequency%(45%)=(%d+)");
     s = s/1000000
     log.debug("[ScannerTemplate]",string.format("scaled freq %d",s))
     properties.cpu_freq.value = s
--  set property volts
     s = voltCmd:read("*a")
     log.debug("[ScannerTemplate]",string.format("raw volts %s", s))
     s = string.match(s,"volt=(%d+\.%d+)");
     log.debug("[ScannerTemplate]",string.format("scaled volts %.1f", s))
     properties.cpu_volt.value = s
end
tasks.refreshProperties = function(me)
    log.trace("[ScannerTemplate]","~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ In tasks.refreshProperties~~~~~~~~~~~~~ ")
queryHardware()
end
